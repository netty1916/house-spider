...
area_dict = dict()
tianjin_dict = ['heping', 'nankai', 'hexi', 'hebei', 'hedong', 'hongqiao', 'xiqing', 'beichen',
                'dongli', 'jinnan', 'wuqing', 'binhaixinqu', 'baodi', 'jizhou', 'jinghai', 'ninghe']
tianjin_dict_plus = ['xiqing', 'beichen', 'dongli', 'jinnan', 'wuqing']
...
def get_districts(city, all=True):
    """
    获取各城市的区县中英文对照信息
    :param city: 城市
    :return: 英文区县名列表
    ...
    if city == 'tj' and not all:
        district_dict2 = {key: chinese_city_district_dict[key]
                          for key in tianjin_dict_plus if key in chinese_city_district_dict}
        en_names = tianjin_dict_plus
        chinese_city_district_dict.clear()
        chinese_city_district_dict.update(district_dict2)
